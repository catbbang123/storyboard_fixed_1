...
const GITHUB_ICON_BASE_URL="https://raw.githubusercontent.com/catbbang123/storyboard_fixed_1/main/icons/";const PERIOD_ICONS=["white.png","red.png","orange.png","yellow.png","green.png","skyblue.png","blue.png","purple.png","black.png","rainbow.png"];function calculateMonthsSinceSignup(e){const t=new Date(e),n=new Date();let s=(n.getFullYear()-t.getFullYear())*12+(n.getMonth()-t.getMonth());return n.getDate()<t.getDate()&&s--,s<0?0:s}function getIconFileNameByPeriod(e){return e>=9?PERIOD_ICONS[9]:PERIOD_ICONS[e]}function getUserIconUrl(e){if(!e||!e.createdAt)return GITHUB_ICON_BASE_URL+PERIOD_ICONS[0];const t=calculateMonthsSinceSignup(e.createdAt);return t>=9&&e.customIconUrl?e.customIconUrl:GITHUB_ICON_BASE_URL+getIconFileNameByPeriod(t)}function applyPersonalMonthlyIcons(){let e=localStorage.getItem("my_platform_join_date");e||(e=new Date().toISOString(),localStorage.setItem("my_platform_join_date",e));const t={nickname:localStorage.getItem("my_platform_nickname")||"창작자",createdAt:e,customIconUrl:localStorage.getItem("my_custom_icon_path")},n=calculateMonthsSinceSignup(t.createdAt),s=getUserIconUrl(t),o=n>=9||localStorage.getItem("my_custom_icon_path");document.querySelectorAll("*").forEach(e=>{if(e.closest("#googleLoginBtn")||e.closest("#profileBtn")||e.closest("#profileMenu")||"profileName"===e.id)return;if(0===e.children.length&&e.textContent){const t=e.textContent.trim(),r=t.includes("👤"),c=t.endsWith("명")&&!t.includes("아이콘");(r||c)&&(e.dataset.iconApplied==="true"&&e.dataset.currentSrc===s||(e.innerHTML="",i=document.createElement("img"),i.src=s,i.className="dynamic-user-icon",i.style.width="16px",i.style.height="16px",i.style.marginRight="4px",i.style.verticalAlign="middle",i.style.objectFit="contain",o?(i.style.cursor="pointer",i.title=`클릭하여 500x500 아이콘 변경하기 (활동 경과: ${n}개월)`,i.onclick=e=>{e.stopPropagation(),openIconChangeModal()}):i.title=`활동 경과: ${n}개월 차 (9개월 차부터 아이콘 변경 권한이 주어집니다!)`,e.appendChild(i),e.append(` ${t.replace("👤","").trim()}`),e.dataset.iconApplied="true",e.dataset.currentSrc=s))}})}function openIconChangeModal(){let e=document.getElementById("iconChangeModal");e&&e.remove();document.body.insertAdjacentHTML("beforeend",'\n        <div id="iconChangeModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display: flex; justify-content: center; align-items: center; z-index: 9999;">\n            <div style="background: white; padding: 25px; border-radius: 12px; width: 350px; text-align: center; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">\n                <h3 style="margin-top: 0; color: #333; font-size: 18px;">✨ 나만의 아이콘 변경</h3>\n                <p style="font-size: 13px; color: #666; margin-bottom: 15px;">500 x 500 px 규격의 이미지를 업로드해 주세요.</p>\n                \n                <input type="file" id="iconFileInput" accept="image/*" style="margin-bottom: 15px; width: 100%; font-size: 12px;">\n                \n                <div style="margin-bottom: 15px;">\n                    <img id="iconPreview" src="" style="width: 80px; height: 80px; object-fit: contain; border: 1px dashed #ccc; border-radius: 8px; display: none; margin: 0 auto;" alt="미리보기">\n                </div>\n\n                <div style="display: flex; gap: 10px; justify-content: center;">\n                    <button id="saveIconButton" style="background: #6c5ce7; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: bold;">저장하기</button>\n                    <button id="closeIconButton" style="background: #b2bec3; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer;">닫기</button>\n                </div>\n            </div>\n        </div>\n    ');const e=document.getElementById("iconFileInput"),t=document.getElementById("iconPreview");let n="";e.onchange=e=>{const s=e.target.files[0];s&&(tag=new FileReader,tag.onload=e=>{n=e.target.result,t.src=n,t.style.display="block"},tag.readAsDataURL(s))},document.getElementById("saveIconButton").onclick=()=>{n?(localStorage.setItem("my_custom_icon_path",n),alert("아이콘이 성공적으로 변경되었습니다!"),document.getElementById("iconChangeModal").remove(),applyPersonalMonthlyIcons()):alert("변경할 이미지를 선택해 주세요!")},document.getElementById("closeIconButton").onclick=()=>{document.getElementById("iconChangeModal").remove()}}document.addEventListener("DOMContentLoaded",()=>{applyPersonalMonthlyIcons()}),setInterval(()=>{applyPersonalMonthlyIcons()},500),window.testMonthsLater=function(e){const t=new Date;t.setMonth(t.getMonth()-e),localStorage.setItem("my_platform_join_date",t.toISOString()),applyPersonalMonthlyIcons(),console.log(`[테스트 완료] 가입 후 ${e}개월 차로 설정되었습니다.`)};
